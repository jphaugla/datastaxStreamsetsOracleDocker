# Streaming from Oracle to DataStax with StreamSets
Provides a quick-start example of streaming data from Oracle to DataStax Enterprise (DSE) with StreamSets Data Collector.


## Overview
In this tutorial, the StreamSets Data Collector is used to create streaming data pipelines to read from Oracle and write to DSE.  The high-level architecture is shown in the following diagram:
...
### Enable access to hr schema 

1. copy script to oracle container login to the orcle container using bash
```bash
docker cp openOracle.sh orcl:/
docker exec -u oracle -it orcl bash
```
2. Login to sqlplus with admin privileges
```bash
sqlplus "/ as sysdba"
```
3. run following alter user commands to unlock HR and give priveleges to HR
```bash
ALTER USER hr ACCOUNT UNLOCK;
ALTER USER hr IDENTIFIED BY datastax;
GRANT create session, alter session, execute_catalog_role, select any dictionary, select any transaction, select any table to hr;
GRANT select on v_$logmnr_parameters to hr;
GRANT select on v_$logmnr_logs to hr;
GRANT select on v_$archived_log to hr;
exit
```

###  Turn on archive logging

1.  log back in to sqlplus
```bash
sqlplus "/ as sysdba"
```
...
shutdown immediate;
```
3. Start up and mount the database:
```bash
startup mount;
```
4. Configure enable archiveing and open the database:
```bash
...
###  Extract a Log Miner Dictionary (Redo Logs)
